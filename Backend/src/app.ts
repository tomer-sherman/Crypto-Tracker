...
import { cryptoTrackerMcpServer } from "./ai-mcp-layer/mcp-server";
...
class App {
    ...
    public start(): void {
        ...
        // Trust the ngrok proxy so rate-limiting sees the real client IP:
        ...
        //Register mcp Server:
        const mcpServer = cryptoTrackerMcpServer.create();
        const mcpController = sseHandlers(() => mcpServer, {});
        server.get("/sse", mcpController.getHandler);
        server.post("/messages", mcpController.postHandler);
